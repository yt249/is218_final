...
if(isset($_POST['submit'])){
	...
	$insertQuery = "INSERT INTO 218Task (username, title, descr, dueDate, urgency, completed)
              	    VALUES ('$username', '$taskname', '$taskdescrip', '$duedate', '$taskurgency', true);";
	try{
...
<div class = "col-2 col-8">
    ...
    <p class = "text justify-content-evenly">
		...
		$completedQuery = "SELECT * FROM 218Task WHERE userName='$username' AND completed = true ORDER BY ".$order." ".$ascdesc;
        try{
    		//display uncomplete tasks
    		foreach (runQuery($completedQuery, $conn) as $result){
    			if ($result['urgency']==0){
    				$prioritydata = 'Normal';
